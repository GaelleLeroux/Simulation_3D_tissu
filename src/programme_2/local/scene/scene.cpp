...
void scene::restart_animation()
{
    mesh_cloth.reset_cloth();
}

void scene::change_raideur(int K)
{
    Kr = K;
}

void scene::change_vent(float Kw)
{
    Kwind = Kw;
}

void scene::change_temps(float T)
{
    delta_t = T;
}

void scene::change_attenuation(float A)
{
    mu = A;
}
...
void scene::draw_scene()
{
    ...
    try
    {
        if(divergence==false && time_integration.elapsed() > 5)
        {
            ...
            mesh_cloth.update_force(Kwind);
            // std::cout<<"Temps delta_t : "<<delta_t<<std::endl;

            mesh_cloth.integration_step(delta_t, mu);
