...
void myWidgetGL::restart_animation()
{
    scene_3d.restart_animation();
}

void myWidgetGL::change_raideur(int K)
{
    select_K = K;
    scene_3d.change_raideur(select_K);
    update();
}

void myWidgetGL::change_vent(int Kw)
{
    // conversion de Kw en float
    select_Kw = Kw / 1000.0f;
    scene_3d.change_vent(select_Kw);
    update();
}

void myWidgetGL::change_temps(int T)
{
    // conversion de T en float
    select_T = T / 1000.0f;
    scene_3d.change_temps(select_T);
    update();
}

void myWidgetGL::change_attenuation(int A)
{
    // conversion de A en float
    select_A = A / 1000.0f;
    scene_3d.change_attenuation(select_A);
    update();
}